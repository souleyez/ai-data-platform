...
- 统一 chat 输出结构字段约定
- 当前阶段先坚持低风险方案：先完成手头的分类/召回/真实材料回归主线，不引入 OpenMAIC 大体量能力，也不直接搬其代码
- 把 OpenMAIC 的可借鉴部分仅纳入后续计划：参考其 PDF provider abstraction 设计，在本项目中未来自实现一个轻量 provider 接口，当前先保留 `pdf-parse` 为默认实现，并预留后续 `mineru` 扩展位
- 将 MinerU 明确纳入备选架构：适用于复杂 PDF、OCR、表格/公式、布局保留需求上升后的增强解析方案；当前阶段只做架构预留，不在手头主线尚未收口时直接接入
