...
export function countTopValues(values: string[], limit = 6) {
...
export function collectStructuredValues(profile: Record<string, unknown> | null | undefined, keys: string[]) {
...
export function detectConceptDimension(requestText: string, templateTaskHint?: KnowledgeTemplateTaskHint | null) {
...
export function buildConceptSections(dimension: string, templateTaskHint?: KnowledgeTemplateTaskHint | null) {
  if (templateTaskHint === 'resume-comparison') {
...
export function buildConceptGroupingHints(
  ...
  const values = documents.flatMap((item) => {
    ...
    return [...(item.topicTags || []), String(item.schemaType || item.category || 'generic').trim()].filter(Boolean);
  });
