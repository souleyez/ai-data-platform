...
export function normalizePersistedAccessKeyState(value: unknown): AccessKeyState {
...
async function readState() {
  ...
  try {
    ...
    const state = normalizePersistedAccessKeyState(JSON.parse(raw));
    const migrated = JSON.stringify(state) !== JSON.stringify(JSON.parse(raw));
