...
export function detectKind(prompt: string): DatasourceKind {
...
export function detectAuthMode(prompt: string, kind: DatasourceKind): DatasourceAuthMode {
  const text = normalizeText(prompt);
...
export function detectSchedule(prompt: string) {
  const text = normalizeText(prompt);
...
export function matchPresets(prompt: string, presets: DatasourcePreset[]) {
  const text = normalizeText(prompt);
  return presets
    .map((preset) => {
      let score = 0;
...
export function buildFallbackTargetLibraries(
  prompt: string,
...
export function buildFallbackName(prompt: string, kind: DatasourceKind, presetMatches: DatasourcePreset[]) {
...
export function buildFallbackDraft(
  prompt: string,
  fallbackKind: DatasourceKind,
  fallbackAuth: DatasourceAuthMode,
  fallbackSchedule: { kind: 'manual' | 'daily' | 'weekly'; timezone: string; maxItemsPerRun: number },
  fallbackTargets: DatasourceTargetLibrary[],
  presetMatches: DatasourcePreset[],
  libraryMatches: Array<{ library: { key: string; label: string }; score: number }>,
) {
