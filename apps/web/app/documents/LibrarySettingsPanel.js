...
export default function LibrarySettingsPanel({
  ...
  if (!activeLibraryRecord || !activeLibrarySettingsDraft) return null;
  ...
  return (
    <section className="card documents-card library-settings-panel-shell">
      <div className="panel-header">
        <div>
          <h3>数据集详情</h3>
          <p>这里维护当前数据集的名称、文档等级、知识页策略和结构化提取规则。编辑区会在页面下方展开，不再挤进顶部工具条。</p>
        </div>
      </div>

      <div className="library-settings-panel-grid">
      ...
      <div className="bot-config-actions">
        ...
        >
          {settingsSubmittingId === activeLibraryRecord.key ? '保存中...' : '保存数据集设置'}
        </button>
      </div>
    </section>
  );
